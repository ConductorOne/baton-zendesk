...
<Note>
**Account entitlements.** Accounts carry no entitlements of their own — a user's organization access is expressed as grants against Organizations. When Organizations are excluded from the sync, the connector skips the account grant pass as well, since the organizations those grants point at wouldn't be present.
</Note>
